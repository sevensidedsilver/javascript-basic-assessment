...
for (var key in snake) {
  if (snake.hasOwnProperty(key) && key[0] === 's') {
    snake[key] = 's';
  }
}
...
function doubleTheFun(para){
	if (typeof para === 'number' || parseInt(para) > 0) {
		return para * 2;
...
// #14 Create a function called coderTest that takes in an object that is a person. It looks to see if the person’s name is Jeremy and then changes the person object to have a property called ç with a value of 10.  If their name is Brack set lovesCode to 0.  otherwise set lovesCode to 5.
...
var outside = function(temp, hum, cloud) {
	if (temp > 80 && hum > 40) {
